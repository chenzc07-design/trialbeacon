 ...
 * A SINGLE PayPalScriptProvider wraps both buttons — PayPal's JS SDK may only
 * be initialised once per page, so two providers would collide and one of the
 * buttons would never render.
...
export function ProPayments() {
  ...
  const singleCard = (
    <div className="card flex flex-col p-5">
      ...
      <div className="mt-4 flex-1">{PAYPAL_ON ? singleBtn : unavailable}</div>
    </div>
  );
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      {PAYPAL_ON ? (
        <PayPalScriptProvider
          options={{ clientId: CLIENT_ID!, currency: 'USD', intent: 'subscription', vault: true }}
        >
          {singleCard}
          {monthlyCard}
          {reserved}
